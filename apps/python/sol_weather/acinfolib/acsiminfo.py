...
WBR, Rombik :)
"""
# based on sim_info.py
# striped down and renamed classes to avoid probs with other sim_info.py
...
class SPageFilePhysics2(ctypes.Structure):
    _pack_ = 4
    _fields_ = [
        ('packetId', c_int32),
        ('gas', c_float),
...
class SPageFileGraphic2(ctypes.Structure):
    _pack_ = 4
    _fields_ = [
        ('packetId', c_int32),
        ('status', AC_STATUS),
...
class SPageFileStatic2(ctypes.Structure):
    _pack_ = 4
    _fields_ = [
...
class ACSimInfo:
    def __init__(self):
        self._acpmf_physics = mmap.mmap(0, ctypes.sizeof(SPageFilePhysics2), "acpmf_physics")
        self._acpmf_graphics = mmap.mmap(0, ctypes.sizeof(SPageFileGraphic2), "acpmf_graphics")
        self._acpmf_static = mmap.mmap(0, ctypes.sizeof(SPageFileStatic2), "acpmf_static")  ### hier stand acpmf_graphics :(
        self.physics = SPageFilePhysics2.from_buffer(self._acpmf_physics)
        self.graphics = SPageFileGraphic2.from_buffer(self._acpmf_graphics)
        self.static = SPageFileStatic2.from_buffer(self._acpmf_static)
